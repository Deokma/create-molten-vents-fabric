package com.deokma.MoltenVents.api.biome.features.types;
...
public class MoltenVentFeature extends Feature<MoltenVentConfiguration> {
    ...
    public boolean generate(FeatureContext<MoltenVentConfiguration> context) {
        ...
        ServerWorld level = context.getWorld().toServerWorld();
        BlockStateProvider decorativeBlock = config.getDecorativeBlock();
        BlockStateProvider outerBlock = config.getOuterBlock();
        BlockStateProvider innerBlock = config.getInnerBlock();
        BlockStateProvider liquidBlock = config.getLiquidBlock();
        boolean underwater = config.isUnderwater();
        int depth = config.getDepth().get(random);

        if (depth > 80 && underwater) {
            depth = 80;
        }

        boolean suitableEnvironment = underwater ? level.getBlockState(origin.up(2)).isOf(Blocks.WATER)
        ...
        if (suitableEnvironment && level.getBlockState(origin.down()).isSolid()) {
            ...
            level.setBlockState(origin, underwater ?
                    Blocks.WATER.getDefaultState() : Blocks.AIR.getDefaultState(), 3);
            level.setBlockState(origin.down(), innerBlock.get(random, origin.down()), 3);
            return true;
        }
        return false;
    }

    private void placeStraight(ServerWorld level, Random random, BlockPos pos,
    ...
    private void placeDiagonal(ServerWorld level, Random random, BlockPos pos,
                               BlockStateProvider block, int min, int max, double prob,
                               @Nullable BlockStateProvider deco) {
        for (int r = min - 1; r < max; r++) {
    ...
    private void placePartialDiagonal(ServerWorld level, Random random, BlockPos pos,
    ...
    private void placeBlock(ServerWorld level, Random random, BlockPos pos,
